   ...
   function handlePurchaseButtonClick(e) {
    ...
    function handleUserPurchaseSubmit(e) {
        ...
        const validators = {
                ...
                tiktok: {
                    profile: link =>
                        // @username direto, URL de perfil/live, com ou sem http(s):// e www.
                        /^@?[\w.-]+$/.test(link) ||
                ...
                facebook: {
                    profile: link =>
                        ...
                        // 5. Links de Grupo: facebook.com/groups/NOME_OU_ID_DO_GRUPO/
                        /^https:\/\/(www\.)?facebook\.com\/groups\/[A-Za-z0-9._-]+\/?(?:\?.*)?$/.test(link),
                
                    post: link =>
                        // 1. Post padrão: facebook.com/username/posts/IDPOST
                        /^https:\/\/(www\.)?facebook\.com\/[A-Za-z0-9._-]+\/posts\/\d+\/?(?:\?.*)?$/.test(link) ||
                
                        // 2. Links de Foto (via fbid)
                        ...
                        // 4. Links de Vídeo (Watch): facebook.com/watch/?v=VIDEO_ID
                        /^https:\/\/(www\.)?facebook\.com\/watch\/?\?(?:[^#]*&)?v=\d+(?:&[^#]*)?$/.test(link) ||
                
                        // 5. NOVO: Links de compartilhamento curtos (facebook.com/share/r/...)
                        /^https:\/\/(www\.)?facebook\.com\/share\/r\/[A-Za-z0-9]+\/?(?:\?.*)?$/.test(link)
                },
                twitch: {
                    profile: link =>
                        // 1. Apenas username: @username ou username
                        /^@?[\w]+$/.test(link) ||
                        // 2. URL de perfil: https://www.twitch.tv/username/ (Este DEVE funcionar aqui)
                        /^https:\/\/(www\.)?twitch\.tv\/\w+\/?$/.test(link),
                    
                    post: link =>
                        // 1. Links de CLIPS: Requer /clip/ no caminho
                        /^https:\/\/(www\.)?twitch\.tv\/(?:\w+)\/clip\/[\w-]+\/?(?:\?.*)?$/.test(link) ||
                        
                        // 2. Links de VODs por ID: Requer /videos/ seguido de NÚMEROS
                        /^https:\/\/(www\.)?twitch\.tv\/videos\/\d+\/?(?:\?.*)?$/.test(link) ||
                
                        // 3. Páginas de VODs de um canal: Requer /videos no final (ex: /buster/videos)
                        /^https:\/\/(www\.)?twitch\.tv\/\w+\/videos\/?(?:\?.*)?$/.test(link)
                },
            rumble: {
                profile: link =>
