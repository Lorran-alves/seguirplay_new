...
<!-- Modal -->
<div class="modal fade modalShowPost" id="modalPostFirst" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
     aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 shadow-lg">
            <div class="modal-header border-0">
            ...
            <div class="modal-body text-center p-4">
                <!-- Imagem da postagem -->
                <img id="postImage" class="img-fluid rounded mb-3" src="" alt="Postagem" style="max-height: 400px; object-fit: cover;">

                <!-- Quantidade de likes -->
                <div class="mb-2">
                    <strong class="fs-5"><span id="postLikes">0</span> curtidas</strong>
                </div>

                <!-- Descrição da postagem -->
                <p class="text-muted mb-4" id="postDescription"></p>

                <!-- Frase de confirmação -->
                <h5 class="mb-3 text-dark fw-semibold">Essa é a postagem certa?</h5>
...
const urlProcessPaymentCard = "{{ route('web.purchases.processPaymentCard') }}";
const urlRedirectSuccess = "{{ route('web.compraFinalizada') }}";
let urlIspay = "{{ route('web.purchases.ispay', ['purchase' => '#']) }}";
...
function inicializarEventos() {
    ...
    $('#modalImagens .continueButton').click(proximoPasso);
    $("button[cmnt='c']").click(configurarComentarios);
    $("button[data-bs-target='#passo01']").click(configurarEtapa01);
    $("#formPurchase").submit(submeterFormularioCompra);
    $("#btnCard").click(() => alternarModal('#passo04', '#modalAcceptCard'));
    $("#btnPIX").click(handlePixClick);
    $("#button_checkbox").click(verificarCheckbox);
    configurarCupom();
}
...
function verifyIsPostFirst(post, input){
    if(post == 'instagram'){
...
function handleUserPurchaseSubmit(e) {
    ...
    const input = $('#linkEmbed').val();

    if (type === 'profile') {
        $('.modalShowSocial').modal('show');
    } else if (type === 'post') {
        if(verifyIsPostFirst(social, input)){
            $('#modalPostFirst').modal('show')
        }else{
    ...
    if (social === 'instagram') {
        buscarDadosInstagram(type, input, verifyIsPostFirst(social, input));
    }
}

$('.continueButton').click(function(){
    $('.modalShowSocial').modal('hide');
    $('#modalImagens').modal('hide')
    $('#passo02').modal('show')
})
...
function configurarComentarios() {
    ...
    mostrarComentarios = quant > 0;

    if (cookies) ocultarCamposContato();
}

function configurarEtapa01() {
    const islink = $(this).data('islink');
    type = islink;
    $('#divInstagram, #divInstagramLike').hide();

    configurarTextoTitulo(islink);
}

function submeterFormularioCompra(e) {
    e.preventDefault();

    if (!cookies && !validarTelefone()) return;

    const botaoClicado = e.originalEvent.submitter;
...
function handlePixClick(e) {
    ...
    if (cookies) return pixCart();

    $.post($('#formPurchase').attr('action') + '/pix', $('#formPurchase').serializeArray(), function (response) {
        const obj = JSON.parse(response);

        $('#load').hide();
        $('#img-pix').attr('src', 'data:image/jpeg;base64,' + obj.transaction_data.qr_code_base64);
        $('#code-pix').val(obj.transaction_data.qr_code);
        $('#dix-pix').show();
...
// FUNÇÕES DE PROCESSO
function buscarDadosInstagram(type, input, postFirst) {
    const username = clear_link('instagram', input);
    const headers = {
        'x-rapidapi-key': 'e0ba27a379mshb639b98dd6ac766p14242bjsn58622ee88b27',
        'x-rapidapi-host': 'instagram-premium-api-2023.p.rapidapi.com'
    };

    if (type === 'profile') {
        $('.modalShowSocial #imageLoading').show();
        $.ajax({
            url: `https://instagram-premium-api-2023.p.rapidapi.com/v1/user/web_profile_info?username=${username}`,
            method: 'GET',
            headers
        }).done(function (response) {
            addSocialModal(type, response.user.profile_pic_url, response.user.username, response.user.edge_followed_by.count, response.user.edge_follow.count, response.user.biography);
            $('.modalShowSocial #imageLoading').hide();
            
            if(response.user.is_private){
                voltarAoModal(2)
                alert('Seu perfil está privado')
            }
        });
    } else if (type === 'post') {
        if(postFirst){
            $('#modalPostFirst #imageLoading').show();
            $.ajax({
                url: `https://instagram-premium-api-2023.p.rapidapi.com/v1/media/by/url?url=${input}`,
                method: 'GET',
                headers
            }).done(addSocialPostFirstModal);
            
        }else{
            $('#modalImagens #imageLoading').show();
            $.ajax({
                url: `https://instagram-premium-api-2023.p.rapidapi.com/v1/user/web_profile_info?username=${username}`,
                method: 'GET',
                headers
                ...
                $.ajax({
                    url: `https://instagram-premium-api-2023.p.rapidapi.com/v1/user/medias?amount=33&user_id=${response.user.id}`,
                    method: 'GET',
                    headers
                }).done(addSocialPostModal);
            });
        }
    }
}
...
function enviarRequisicao() {
    $.post(urlIspay, function (response) {
        const obj = JSON.parse(response);
        if (obj.status === true) {
...
function addSocialModal(type, image, name, followers, following, description) {
    $('.modalShowSocial #socialImage')
        .attr('src', 'proxy.php?url=' + encodeURIComponent(image))
        .css({ width: '100px', 'border-radius': '100%' });

    $('.modalShowSocial #socialname').html(name);
    $('.modalShowSocial #socialFollowers').html(followers);
    $('.modalShowSocial #socialFollowing').html(following);
    $('.modalShowSocial #socialDescription').html(description);
}

function addSocialPostModal(posts) {
    $('#modalImagens #imageLoading').hide();
    const container = document.getElementById('imageSelection');
    $(posts).each(function (_, post) {
        const col = $('<div class="col-4 col-md-3"></div>');
        const img = $('<img>')
            .attr('src', 'proxy.php?url=' + encodeURIComponent(post.image_versions[0].url))
            .addClass('selectable-img rounded-3 shadow-sm w-100')
            .css({ height: '150px', objectFit: 'cover' })
            .attr('data-url', `https://www.instagram.com.com/p/${post.code}`)
...
function addSocialPostFirstModal(post) {
    $('#modalPostFirst #imageLoading').hide();
    console.log(post.image_versions[0])
    
    $('#modalPostFirst #postImage').attr('src', 'proxy.php?url=' + encodeURIComponent(post.image_versions[0].url));
    $('#modalPostFirst #postLikes').text(post.like_count);
    $('#modalPostFirst #postDescription').text(post.caption_text);
}

function selecionarImagem(img) {
    const alreadySelected = img.hasClass('selected');
    $('.selectable-img').removeClass('selected');

    if (!alreadySelected) {
        img.addClass('selected');
        $('#linkEmbed').val(img.data('url'));
        $('#modalImagens .continueButton').removeClass('d-none');
        $('#passo02 input[name="profile"]').val(img.data('url'));
    } else {
        $('#linkEmbed').val('');
        $('#modalImagens .continueButton').addClass('d-none');
    }
}

function configurarTextoTitulo(islink) {
    if (islink === 3) {
    ...
    $('#linkEmbed').off('change').change(function () {
        const linkBase = islink === 3 ? 'https://www.instagram.com/' : '';
        addLink(linkBase, $(this).val());
    });
}

function addLink(base, val) {
    const link = base + val;
    $('#framelike').attr('href', link);
}

function validarTelefone() {
    const telefone = $('#phone_code').val() + $('#phone').val();
...
function alternarModal(modalEsconder, modalMostrar) {
    $(modalEsconder).modal('hide');
    $(modalMostrar).modal('show');
}

function configurarCupom() {
    const cupomInputEntrada = $('#cupomInputEntrada');
    const campoPrecoOld = $('#totalInstagram');
    const campoPreco = $('#totalPrice');
    const feedbackCupom = $('#feedbackCupom');
    const cupomForm = $('#cupomForm');

    let tempo;
    const intervalo = 1000;

    cupomInputEntrada.keyup(() => {
        clearTimeout(tempo);
        if (cupomInputEntrada.val()) {
            tempo = setTimeout(() => aplicarDesconto(cupomInputEntrada, campoPrecoOld, campoPreco, cupomForm, feedbackCupom), intervalo);
        }
    });

    cupomInputEntrada.blur(() => aplicarDesconto(cupomInputEntrada, campoPrecoOld, campoPreco, cupomForm, feedbackCupom));
}

function aplicarDesconto(cupomInputEntrada, campoPrecoOld, campoPreco, cupomForm, feedbackCupom) {
    campoPrecoOld.removeClass();
    campoPreco.removeClass();

    const cupons = {!! $cupons !!};
    const cupomDigitado = cupomInputEntrada.val().toUpperCase();
    const categoria = $('input[name="categoria_id"]').val();
    const plano = $('input[name="plano_id"]').val();
    ...
    let preco = parseFloat(campoPrecoOld.text().includes(',')
    ...
    $.post("{{ route('web.verificarCupom') }}", {
        ...
        if (response.status === "success") {
            let descontoDecimal = 1 - (response.desconto / 100);
            let totalComDesconto = arredondarParaDuasCasasDecimais(preco * descontoDecimal);
